...
from task_manager import TaskManager, TaskAlreadyCompletedError
...
def test_create_a_simple_task(task_manager):
    ...
    assert isinstance(task.id, int), "Task should have a unique ID of type int"
...
def test_cannot_complete_already_completed_task(task_manager):
    ...
    # Action and Assertions: Attempt to complete the already completed task
    ...
    assert task.completed_at == original_completed_at, "completed_at should not change"
