import pytest
from library_management_system import Library, User, Book, Reservation, LendingRecord, InterLibraryLoan, AccountReview

@pytest.fixture
def setup_library():
    library = Library()
    library.add_book("Python Programming", 3)
    library.add_book("Data Structures", 1)
    library.add_book("Algorithms", 1)
    library.register_user("john_doe")
    library.register_user("jane_smith")
    library.register_user("bob_wilson")
    library.register_user("alex_jones")
    return library

def test_reserve_available_book(setup_library):
    ...
    assert book.available_copies == 2, "Available copies should decrease from 3 to 2"
...
def test_lend_reserved_book(setup_library):
    library = setup_library
    ...
    assert lending_record.late_fee_policy_active, "Late fee policy should be activated for due date"

def test_handle_book_return_with_late_fee(setup_library):
    library = setup_library
    ...
    assert book.needs_inspection, "Book should be marked for inspection before re-shelving"

def test_process_inter_library_loan(setup_library):
    library = setup_library
    user = library.get_user("john_doe")
    
    inter_library_loan = library.request_inter_library_loan(user, "Advanced Calculus")
    ...
    assert user.is_notified_on_arrival, "User should be notified when book arrives"
    assert inter_library_loan.special_loan_terms == (21, False), "Special loan terms should apply (21-day period, no renewal)"

def test_handle_user_suspension(setup_library):
    library = setup_library
    user = library.get_user("alex_jones")
    user.late_fee = 25.00
    user.days_since_last_payment = 30
    
    library.process_account_review(user)
    
    assert user.status == "suspended", "User account status should change to 'suspended'"
    assert not user.has_active_reservations(), "All active reservations should be cancelled"
    assert not user.can_make_new_reservations_or_loans(), "User cannot make new reservations or loans"
    assert user.received_notification_with_payment_instructions, "Notification should be sent with payment instructions"